          ...
          {fotosRutaActual.map((_, index) => (
            <div
              ...
              className={`w-2 h-2 rounded-full ${
                index === indiceSliderFotos ? 'bg-white' : 'bg-white/50'
              }`}
            />
          ))}
        </div>
      ...
      <div className="bg-white rounded-lg shadow-lg p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            ...
            <div className="mt-6">
              <h3 className="text-lg font-semibold text-teal-700 mb-2">
                {t('rutasIncluidas', 'Rutas incluidas')}
              </h3>
              <p className="text-gray-600">
                {rutaActual.rutasIncluidas || t('ningunaRutaAsociada', 'Ninguna ruta asociada')}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center">
              <span className="text-teal-700 font-semibold w-24">{t('fechas', 'Fechas')}:</span>
              <div>
                <div>{t('desde', 'Desde')}: {rutaActual.fechaInicio || t('fechaNoDisponible', 'Fecha no disponible')}</div>
                <div>{t('hasta', 'Hasta')}: {rutaActual.fechaFin || t('fechaNoDisponible', 'Fecha no disponible')}</div>
              </div>
            </div>
