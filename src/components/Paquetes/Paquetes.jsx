...
const GestionPaquetes = () => {
  ...
  const fetchPaquetes = async () => {
    ...
    try {
      const response = await axios.get(
        "https://servicio-explococora.onrender.com/paquete/lista-paquetes"
      );
      ...
      const paquetesConRutas = await Promise.all(
        paquetesUnicos.map(async (paquete) => {
          if (paquete.idPaquete) {
            const rutasAsociadas = await obtenerRutasAsociadas(
              paquete.idPaquete
            );
            return {
              ...paquete,
      ...
      if (paquetesConRutas.length > 0) {
        if (idPaquete) {
          ...
          if (paqueteSeleccionado) {
            ...
            console.warn(
              `No se encontró paquete con ID ${idPaquete}, usando el primero disponible`
            );
  ...
  const obtenerFotosPaquete = async (idPaquete) => {
    try {
      ...
      const response = await axios.get(
        `https://servicio-explococora.onrender.com/paquete/fotos-publicas/${idPaquete}`
      );
  ...
  const obtenerRutasAsociadas = async (idPaquete) => {
    try {
      ...
      const response = await axios.get(
        `https://servicio-explococora.onrender.com/paquete/obtener-paquete/${idPaquete}`
      );
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        if (response.data.idRuta || response.data.nombreRuta) {
          rutasInfo.push({
            ...
            nombreRuta:
              response.data.nombreRuta || `Ruta ${response.data.idRuta}`,
            tiempoEstimado:
              response.data.duracion || response.data.tiempoEstimado || null,
          });
        }
  ...
  const calcularDuracionTotalPaquete = (paquete) => {
    if (!paquete?.rutasAsociadas || !paquete.rutasAsociadas.length)
      return paquete?.duracion || "No especificada";
    ...
    let duracionTotal = "";
    if (horasTotales > 0)
      duracionTotal += `${horasTotales} hora${horasTotales !== 1 ? "s" : ""}`;
    if (minutosTotales > 0) {
      if (duracionTotal) duracionTotal += " ";
      duracionTotal += `${minutosTotales} minuto${
        minutosTotales !== 1 ? "s" : ""
      }`;
    }
  ...
  const renderSlider = () => {
    const fotos =
      paqueteActual && paquetesConFotos[paqueteActual.idPaquete]
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 text-white p-3 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-y-2 group-hover:translate-y-0 z-20">
                  <p className="text-sm font-medium">
                    {paqueteActual?.nombrePaquete} • Foto {index + 1}
                  </p>
                </div>
              </div>
            </div>
  ...
  if (!paqueteActual) {
    return <div className="text-center p-4">No hay paquetes disponibles</div>;
  }
              ...
              <div className="mb-10 mx-4">
                <div className="relative pl-4 py-2">
                  ...
                  <p className="mt-2 text-gray-700 text-base font-normal">
                    &ldquo;Este paquete ofrece una experiencia completa que te
                    permitirá disfrutar de las mejores atracciones del Valle del
                    Cocora, combinando aventura y naturaleza durante tu
                    visita.&rdquo;
                  </p>
                </div>
              </div>
                  ...
                  <div className="bg-gradient-to-br from-emerald-50/90 via-white to-emerald-50/70 p-6 rounded-2xl border border-emerald-100 h-full flex flex-col shadow-[0_10px_25px_-12px_rgba(13,148,136,0.25)] relative overflow-hidden backdrop-blur-sm">
                    ...
                    <div className="absolute -top-12 -right-12 w-56 h-56 opacity-[0.03] rotate-12">
                      ...
                      >
                        ...
                        />
                        <path
                          d="M50 10V90M10 50H90M26 26L74 74M26 74L74 26"
                          stroke="#0d9488"
                          strokeWidth="0.4"
                        />
                      </svg>
                    ...
                    <div className="flex-grow grid grid-cols-1 gap-3">
                      ...
                      <div className="bg-gradient-to-br from-white to-emerald-50/50 p-3 rounded-xl border border-emerald-100/80 hover:shadow-md group transition-all duration-300 transform hover:-translate-y-0.5 hover:border-emerald-200">
                        <div className="flex items-center justify-between relative z-10">
                          <div className="flex items-center">
                            ...
                            <div className="ml-3">
                              <span className="text-emerald-600 text-xs font-medium block leading-tight">
                                Duración
                              </span>
                      ...
                      <div className="bg-gradient-to-br from-white to-emerald-50/50 p-3 rounded-xl border border-emerald-100/80 hover:shadow-md group transition-all duration-300 transform hover:-translate-y-0.5 hover:border-emerald-200">
                        <div className="flex items-center justify-between relative z-10 mb-2">
                          <div className="flex items-center">
                            ...
                            <div className="ml-3">
                              ...
                              <span className="text-gray-800 text-base tracking-wide block group-hover:text-emerald-800 transition-colors duration-500">
                                {paqueteActual.rutasAsociadas?.length || 0}{" "}
                                rutas
                              </span>
                            </div>
                          </div>
                        ...
                        <div className="mt-3 pr-1 space-y-2">
                          {paqueteActual.rutasAsociadas &&
                          paqueteActual.rutasAsociadas.length > 0 ? (
                            paqueteActual.rutasAsociadas.map((ruta, index) => (
                              <div
                                key={index}
                                className="flex items-start bg-white/70 p-2 rounded-lg"
                              >
                    ...
                    <div className="space-y-6 pr-2">
                      ...
                      <div className="flex items-start bg-white/70 p-4 rounded-lg shadow-sm hover:shadow-md transition-all duration-300">
                        ...
                        <div>
                          ...
                          <p className="text-gray-700 text-sm mt-1">
                            Hogar del árbol nacional de Colombia, la majestuosa
                            Palma de Cera puede alcanzar hasta 60 metros de
                            altura. Estas imponentes palmas crean un paisaje
                            surrealista durante los recorridos.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start bg-white/70 p-4 rounded-lg shadow-sm hover:shadow-md transition-all duration-300">
                        ...
                        <div>
                          ...
                          <p className="text-gray-700 text-sm mt-1">
                            Recorre senderos ecológicos de diferentes niveles de
                            dificultad. El circuito principal 5 km cruza ríos
                            con puentes colgantes y te lleva a miradores con
                      ...
                      <div className="flex items-start bg-white/70 p-4 rounded-lg shadow-sm hover:shadow-md transition-all duration-300">
                        ...
                        <div>
                          ...
                          <p className="text-gray-700 text-sm mt-1">
                            Explora el valle a lomos de caballos criollos
                            dóciles y entrenados. Las cabalgatas duran entre 1 y
                            3 horas y permiten acceder a zonas que serían
                            difíciles a pie mientras disfrutas del paisaje.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start bg-white/70 p-4 rounded-lg shadow-sm hover:shadow-md transition-all duration-300">
                        ...
                        <div>
                          ...
                          <p className="text-gray-700 text-sm mt-1">
                            Programa tu visita temprano (antes de las 9 a.m.)
                            para evitar la neblina y lluvias de la tarde. Lleva
                            calzado impermeable, ropa abrigada, y repelente.
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-center">
              <div>
                <p className="font-serif text-white text-lg">
                  &ldquo;Descubre la belleza natural del Valle del Cocora con
                  nuestros paquetes turísticos&rdquo;
                </p>
              </div>
