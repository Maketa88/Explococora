export const DetalleServiciosAdicionalesPaquete = ({ servicios }) => {
      ...
      {servicios.map((item, index) => (
        <div key={index} className="flex justify-between py-1 text-gray-700">
          ...
          <span className="font-medium">
            $
            {parseInt(item.servicio.precio * item.cantidad).toLocaleString(
              "es-CO"
            )}
          </span>
        </div>
...
export default DetalleServiciosAdicionalesPaquete;
