...
export const ConfirmacionPagoPaquete = () => {
  ...
  useEffect(() => {
    if (location.state?.pagoSimulado) {
      ...
      setReservaInfo({
        ...
        mensajeConfirmacion:
          location.state.mensajeConfirmacion || "Tu reserva ha sido confirmada",
      ...
      if (reservaPendiente) {
        ...
        setError(
          t("noReservaInfo", "No se encontró información de la reserva")
        );
      }
    }
  }, [location, t]);
  ...
  const calcularTotalServiciosAdicionales = () => {
    if (
      ...
      !location.state.serviciosAdicionales.length
    ) {
      return 0;
    }
  ...
  const calcularPrecioTotal = () => {
    ...
    const precioPaquete =
      (reservaInfo.paqueteInfo.precio || 0) *
                    ...
                    <div className="flex justify-between py-1 text-gray-700">
                      <span>
                        ...
                        <span className="text-gray-500">
                          {" "}
                          x {reservaInfo.paqueteInfo.cantidadPersonas || 1}
                        </span>
                      </span>
                      <span className="font-medium">
                        $
                        {parseInt(
                          reservaInfo.paqueteInfo.precio *
                            (reservaInfo.paqueteInfo.cantidadPersonas || 1)
                  ...
                  location.state.serviciosAdicionales.length > 0 && (
                    <>
                      ...
                      {location.state.serviciosAdicionales.map(
                        (item, index) => (
                          ...
                          >
                            <span>
                              {formatValue(
                                item.servicio?.nombre || item.nombre
                              )}{" "}
                              x {item.cantidad}
                            </span>
                            <span className="font-medium">
                              $
                              {parseInt(
                                (item.servicio?.precio || item.precio) *
                                  item.cantidad
                              ).toLocaleString("es-CO")}
                            </span>
                          </div>
                      ...
                      <div className="flex justify-between py-1 text-green-700 font-medium border-t border-dotted border-gray-200 mt-1 pt-1">
                        ...
                        <span>
                          $
                          {calcularTotalServiciosAdicionales().toLocaleString(
                            "es-CO"
                          )}{" "}
                          COP
                        </span>
                  ...
                  <span className="text-green-600">
                    ...
                    {(
                      (reservaInfo?.paqueteInfo
                        ? reservaInfo.paqueteInfo.precio *
                          (reservaInfo.paqueteInfo.cantidadPersonas || 1)
                        : 0) + calcularTotalServiciosAdicionales()
                    ).toLocaleString("es-CO")}{" "}
                    COP
                  </span>
                </div>
                  ...
                  <p className="text-sm text-gray-700">
                    {reservaInfo?.informacionPago ||
                      (esEfectivo
                        ? t(
                            "notaEfectivo",
                            "Recuerda que deberás pagar en efectivo al momento de tomar el paquete turístico. Ten lista la cantidad exacta para facilitar el proceso."
                          )
                        : t(
                            "notaSimulacion",
