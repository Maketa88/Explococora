...
const MapaRutaIntegracion = ({ rutaPrincipal, altura = "400px" }) => {
  ...
  useEffect(() => {
    ...
    const cargarCoordenadas = async () => {
      try {
        ...
        const coordenadasRuta = await obtenerCoordenadasRuta(
          rutaPrincipal.idRuta
        );
        ...
        setError(
          t("errorCoordenadas", "Error al cargar las coordenadas de la ruta")
        );
      } finally {
        setCargando(false);
      }
    ...
    return (
      <div className="bg-gray-50 rounded-xl p-8 text-center">
        <p className="text-gray-500">
          {t("rutaNoDisponible", "Ruta no disponible")}
        </p>
      </div>
    );
  }
  ...
  return (
    <motion.div
      ...
      className="mb-8"
    >
      <div className="bg-gradient-to-br from-teal-50 via-white to-teal-50   shadow-xl  relative overflow-hidden">
        ...
        <div className="mt-4 text-sm text-gray-500 text-center relative z-10">
          <p>
            {t(
              "mapaInfo",
              "Mapa interactivo proporcionado por Azure Maps. Puedes hacer zoom y mover el mapa para explorar mejor la ruta."
            )}
          </p>
        </div>
      </div>
