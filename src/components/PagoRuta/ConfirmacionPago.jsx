...
export const ConfirmacionPago = () => {
  ...
  useEffect(() => {
    ...
    if (location.state?.pagoSimulado) {
      ...
      console.log('Servicios recibidos en confirmación:', serviciosRecibidos);
      ...
      setReservaInfo({
        ...
        numeroPago: location.state.numeroPago || 'PAG-' + Math.floor(Math.random() * 1000000),
        rutaInfo: location.state.rutaInfo,
        serviciosAdicionales: serviciosRecibidos, // Asegurarnos de usar los servicios recibidos
        tituloConfirmacion: location.state.tituloConfirmacion || 'Pago procesado correctamente',
        mensajeConfirmacion: location.state.mensajeConfirmacion || 'Tu reserva ha sido confirmada con éxito',
  ...
  const verificarEstadoPago = async (radicado) => {
    try {
      ...
      const response = await axios.get(`http://localhost:10101/pagos-rutas/info-reserva`, {
      ...
      console.log('Respuesta de info-reserva:', response.data);
      ...
      if (response.data && response.data.success) {
        ...
        setReservaInfo({
          ...
          serviciosAdicionales: serviciosResponse.length > 0 ? serviciosResponse : location.state?.serviciosAdicionales || []
        });
      } else {
        setEstado('rechazado');
      ...
      setEstado('rechazado');
      setError('No se pudo verificar el estado del pago. Por favor, contacta al soporte.');
    }
  };
          ...
          <div id="ticket-confirmacion" className="max-w-xl mx-auto rounded-lg overflow-hidden shadow-lg border border-gray-200">
            ...
            <div className="bg-teal-800 p-5 text-center">
              ...
              <div className="bg-white rounded-lg p-4 mt-4 mx-auto max-w-md text-left shadow">
                ...
                {reservaInfo?.rutaInfo && (
                  <>
                    <h3 className="font-semibold text-gray-800 mb-2 border-b pb-1">Detalle de la ruta:</h3>
                    <div className="flex justify-between py-1 text-gray-700">
                      <span>
                        {formatValue(reservaInfo.rutaInfo.nombreRuta)}
                        <span className="text-gray-500">x {reservaInfo.rutaInfo.cantidadPersonas || 1}</span>
                      </span>
                      <span className="font-medium">${parseInt(reservaInfo.rutaInfo.precio * (reservaInfo.rutaInfo.cantidadPersonas || 1)).toLocaleString('es-CO')}</span>
                    </div>
                    <div className="flex justify-between py-1 text-green-700 font-medium border-t border-dotted border-gray-200 mt-1 pt-1">
                      ...
                      <span>${parseInt(reservaInfo.rutaInfo.precio * (reservaInfo.rutaInfo.cantidadPersonas || 1)).toLocaleString('es-CO')} COP</span>
                    </div>
                  </>
                )}
                  ...
                  <>
                    ...
                    {location.state.serviciosAdicionales.map((item, index) => (
                      <div key={index} className="flex justify-between py-1 text-gray-700">
                        <span>
                          {formatValue(item.servicio.nombre)} x {item.cantidad}
                        </span>
                    ...
                    <div className="flex justify-between py-1 text-green-700 font-medium border-t border-dotted border-gray-200 mt-1 pt-1">
                      ...
                      <span>${parseInt(location.state.serviciosAdicionales.reduce((total, item) => 
                        total + (item.servicio.precio * item.cantidad), 0)).toLocaleString('es-CO')} COP</span>
                    </div>
                  </>
                )}
                ...
                <div className="flex justify-between mt-3 pt-2 text-lg font-bold border-t-2 border-gray-300">
                  ...
                  <span className="text-green-600">
                    ${(
                      (reservaInfo?.rutaInfo ? (reservaInfo.rutaInfo.precio * (reservaInfo.rutaInfo.cantidadPersonas || 1)) : 0) +
                      (location.state?.serviciosAdicionales ? location.state.serviciosAdicionales.reduce((total, item) => 
            ...
            <div className="bg-white p-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200 mb-5">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  {reservaInfo.fechaInicio && (
                    <div>
                      <p className="text-gray-600 font-medium">Fecha de Ruta:</p>
                  ...
                  {reservaInfo.guiaAsignado && (
                    <div className="col-span-2">
                      ...
                      <p className="font-semibold text-green-700">{formatValue(reservaInfo.guiaAsignado)}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="border-t border-gray-200 pt-4 mt-4 text-center text-sm text-gray-600">
                ...
                <p className="mt-1">Si tienes preguntas, contáctanos a <a href="mailto:info@explococora.com" className="text-green-600">info@explococora.com</a></p>
                <div className="flex justify-center items-center mt-3 text-xs">
                  <a href="https://explococora.com" className="text-green-600">explococora.com</a>
                  <span className="mx-2 text-gray-400">|</span>
                  <a href="https://explococora.com/contacto" className="text-green-600">Contacto</a>
                </div>
                <p className="font-medium mt-3 text-green-600">¡Gracias por elegir ExploCocora!</p>
              </div>
            </div>
  ...
  return (
    <div style={{ 
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div id="ticket-confirmacion" className="bg-white rounded-xl shadow-lg p-8">
          {renderContenido()}
        </div>
      </div>
    </div>
