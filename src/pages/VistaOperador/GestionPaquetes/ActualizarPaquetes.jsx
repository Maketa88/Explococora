...
const ActualizarPaquetes = ({ onClose, onUpdated, paquete, rutasDisponibles, mostrarAlerta }) => {
  ...
  const obtenerFotosPaquete = async () => {
    try {
      ...
      mostrarAlerta('No se pudieron cargar las imágenes del paquete', 'error');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      mostrarAlerta('¡Paquete actualizado exitosamente!', 'success');
      ...
      if (err.response?.data?.errors && Array.isArray(err.response.data.errors)) {
        ...
        if (nombreError) {
          ...
          mostrarAlerta(nombreError.msg, 'error');
          return;
        }
        ...
        if (validationErrors.length > 0 && validationErrors[0].msg) {
          ...
          mostrarAlerta(validationErrors[0].msg, 'error');
          return;
        }
      }
      ...
      mostrarAlerta(err.response?.data?.message || err.message || 'Error al actualizar el paquete', 'error');
    } finally {
      setLoading(false);
    }
  };
