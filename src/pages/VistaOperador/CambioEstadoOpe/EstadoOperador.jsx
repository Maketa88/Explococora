...
const EstadoOperador = () => {
  ...
  const obtenerEstadoDesdeServidor = async () => {
    try {
      ...
      if (response.data && response.data.data && response.data.data.estado) {
        ...
        localStorage.setItem('ultimoEstadoOperador', nuevoEstado);
  ...
  const cambiarEstado = async (nuevoEstado) => {
    try {
      ...
      estadoService.setEstado(nuevoEstado);
  ...
  useEffect(() => {
    const iniciarSincronizacion = async () => {
      ...
      try {
        // 1. Primero intentar usar el estado guardado en localStorage
        const estadoGuardado = localStorage.getItem('ultimoEstadoOperador');
